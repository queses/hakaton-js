  ...
  <nav class="navbar app-navbar navbar-toggleable-md">
    ...
    <div class="row navbar-container">
      ...
      <div class="col nav-item dropdown navbar-dropdown d-flex align-items-center justify-content-center">
        <span class="navbar-plain-text">{{ getCompanyName }}</span>
        <router-link class="plain-link" to="/auth/login">Выйти</router-link>
      </div>
    </div>
  </nav>
</template>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      ...mapState({
        companyTitle: (state) => state.app.companyTitle
      }),
      getCompanyName () {
        return this.companyTitle || 'ООО УК'
      }
    },
    methods: {
      ...mapActions([
